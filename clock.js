...
function showTime(timeDiff){
  ...
  }else if((timeDiff < 0) && (hour - timeDiff >= 24)){//もし日本よりも時刻が進んでいる国で、クリックした方の地点が1日早い時
    //一日進める
    //時刻を変える
    hour = (hour - timeDiff) - 24;

    //年・月・日を変える
    if(day == monthArray[month - 1]){//日本の日が月の最終日だったら
      if(month == 12){//12月31日だったら
        year = year + 1;//次の年にする
      }

      month = (month + 1) % 12;//次の月にするために1足してから12で割った余りを求める。
      if(month == 0){
        month += 12;//11月のみ、monthが0となってしまうので、12を足す。
      }
      day = 1;//次の月の始まりの日は1日
    }else{//日本の日が月の最終日でなかったら
      day = day + 1;//日を一日増やすだけで良い。
    }
...
function deleteMarker(){
  for(let i = 0; i < markers.length; i++){
    markers[i].setMap(null);//マーカー削除。(参考：https://www.javadrive.jp/google-maps-javascript/gmarker/index4.html)
  }
